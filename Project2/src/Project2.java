...
public class Project2 {
	...
	public Tree evaluate(Tree tree) {
		...
		if (tree.get_left() == null && tree.get_right() == null) {
		...
		else if (tree.get_info().getToken() == 23) {
			Token t1 = evaluate(tree.get_left()).get_info();
			Token t2 = evaluate(tree.get_right()).get_info();
			if (t1.getType() == 'i' && t2.getType() == 'i') {
				return new Tree(new Token(11, t1.getValue_i()+t2.getValue_i()));
			}
			else if (t1.getType() == 'f' && t2.getType() == 'f') {
				return new Tree(new Token(12, t1.getValue_f() + t2.getValue_f()));
			}
			else if (t1.getType() == 'f') {
				return new Tree(new Token(12, t1.getValue_f() + t2.getValue_i()));
			}
			else return new Tree(new Token(12, t1.getValue_i() + t2.getValue_f()));
		}
		else if (tree.get_info().getToken() == 24) {
			Token t1 = evaluate(tree.get_left()).get_info();
			Token t2 = evaluate(tree.get_right()).get_info();
			if (t1.getType() == 'i' && t2.getType() == 'i') {
				return new Tree(new Token(11, t1.getValue_i() - t2.getValue_i()));
		...
		else if (tree.get_info().getToken() == 25) {
			Token t1 = evaluate(tree.get_left()).get_info();
			Token t2 = evaluate(tree.get_right()).get_info();
			if (t1.getType() == 'i' && t2.getType() == 'i') {
				return new Tree(new Token(11, t1.getValue_i() * t2.getValue_i()));
			...
			Token t2 = evaluate(tree.get_right()).get_info();
			if (t1.getType() == 'i' && t2.getType() == 'i') {
				return new Tree(new Token(12, t1.getValue_i() / t2.getValue_i()));
			}
			else if (t1.getType() == 'f' && t2.getType() == 'f') {
				return new Tree(new Token(12, t1.getValue_f() / t2.getValue_f()));
			}
